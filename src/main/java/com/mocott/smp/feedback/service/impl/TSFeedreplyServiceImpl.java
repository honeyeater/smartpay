...
@Service("tSFeedreplyService")
@Transactional
public class TSFeedreplyServiceImpl extends CommonServiceImpl implements TSFeedreplyServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(TSFeedreplyEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(TSFeedreplyEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(TSFeedreplyEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(TSFeedreplyEntity t){
		Map<String,Object> map = new HashMap<String,Object>();
		map.put("id", t.getId());
		map.put("feedbackid", t.getFeedbackid());
		map.put("serialno", t.getSerialno());
		map.put("reply_content", t.getReplyContent());
		map.put("reply_time", t.getReplyTime());
		map.put("reply_usercode", t.getReplyUsercode());
		map.put("notify_flag", t.getNotifyFlag());
		map.put("notify_time", t.getNotifyTime());
		map.put("notify_content", t.getNotifyContent());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,TSFeedreplyEntity t){
 		sql  = sql.replace("#{id}",String.valueOf(t.getId()));
 		sql  = sql.replace("#{feedbackid}",String.valueOf(t.getFeedbackid()));
 		sql  = sql.replace("#{serialno}",String.valueOf(t.getSerialno()));
 		sql  = sql.replace("#{reply_content}",String.valueOf(t.getReplyContent()));
 		sql  = sql.replace("#{reply_time}",String.valueOf(t.getReplyTime()));
 		sql  = sql.replace("#{reply_usercode}",String.valueOf(t.getReplyUsercode()));
 		sql  = sql.replace("#{notify_flag}",String.valueOf(t.getNotifyFlag()));
 		sql  = sql.replace("#{notify_time}",String.valueOf(t.getNotifyTime()));
 		sql  = sql.replace("#{notify_content}",String.valueOf(t.getNotifyContent()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("t_s_feedreply",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
