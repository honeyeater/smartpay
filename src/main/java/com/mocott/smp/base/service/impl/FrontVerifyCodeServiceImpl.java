...
@Service("frontVerifyCodeService")
@Transactional
public class FrontVerifyCodeServiceImpl extends CommonServiceImpl implements FrontVerifyCodeServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(FrontVerifyCodeEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(FrontVerifyCodeEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(FrontVerifyCodeEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(FrontVerifyCodeEntity t){
		Map<String,Object> map = new HashMap<String,Object>();
		map.put("id", t.getId());
		map.put("username", t.getUsername());
		map.put("phoneno", t.getPhoneno());
		map.put("valid_code", t.getValidCode());
		map.put("create_time", t.getCreateTime());
		map.put("isuse", t.getIsuse());
		map.put("user_time", t.getUserTime());
		map.put("send_time", t.getSendTime());
		map.put("send_content", t.getSendContent());
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("front_verify_code",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
