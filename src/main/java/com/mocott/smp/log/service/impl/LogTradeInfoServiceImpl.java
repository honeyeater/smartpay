...
@Service("logTradeInfoService")
@Transactional
public class LogTradeInfoServiceImpl extends CommonServiceImpl implements LogTradeInfoServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(LogTradeInfoEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(LogTradeInfoEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(LogTradeInfoEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(LogTradeInfoEntity t){
		Map<String,Object> map = new HashMap<String,Object>();
		map.put("id", t.getId());
		map.put("username", t.getUsername());
		map.put("serialno", t.getSerialno());
		map.put("order_code", t.getOrderCode());
		map.put("static_money", t.getStaticMoney());
		map.put("dyn_money", t.getDynMoney());
		map.put("other_money", t.getOtherMoney());
		map.put("back_money", t.getBackMoney());
		map.put("release_money", t.getReleaseMoney());
		map.put("trade_time", t.getTradeTime());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,LogTradeInfoEntity t){
 		sql  = sql.replace("#{id}",String.valueOf(t.getId()));
 		sql  = sql.replace("#{username}",String.valueOf(t.getUsername()));
 		sql  = sql.replace("#{serialno}",String.valueOf(t.getSerialno()));
 		sql  = sql.replace("#{order_code}",String.valueOf(t.getOrderCode()));
 		sql  = sql.replace("#{static_money}",String.valueOf(t.getStaticMoney()));
 		sql  = sql.replace("#{dyn_money}",String.valueOf(t.getDynMoney()));
 		sql  = sql.replace("#{other_money}",String.valueOf(t.getOtherMoney()));
 		sql  = sql.replace("#{back_money}",String.valueOf(t.getBackMoney()));
 		sql  = sql.replace("#{release_money}",String.valueOf(t.getReleaseMoney()));
 		sql  = sql.replace("#{trade_time}",String.valueOf(t.getTradeTime()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("log_trade_info",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
