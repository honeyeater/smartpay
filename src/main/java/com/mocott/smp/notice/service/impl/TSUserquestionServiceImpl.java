...
@Service("tSUserquestionService")
@Transactional
public class TSUserquestionServiceImpl extends CommonServiceImpl implements TSUserquestionServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(TSUserquestionEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(TSUserquestionEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(TSUserquestionEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(TSUserquestionEntity t){
		Map<String,Object> map = new HashMap<String,Object>();
		map.put("id", t.getId());
		map.put("question_title", t.getQuestionTitle());
		map.put("question_content", t.getQuestionContent());
		map.put("question_type", t.getQuestionType());
		map.put("question_level", t.getQuestionLevel());
		map.put("question_term", t.getQuestionTerm());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,TSUserquestionEntity t){
 		sql  = sql.replace("#{id}",String.valueOf(t.getId()));
 		sql  = sql.replace("#{question_title}",String.valueOf(t.getQuestionTitle()));
 		sql  = sql.replace("#{question_content}",String.valueOf(t.getQuestionContent()));
 		sql  = sql.replace("#{question_type}",String.valueOf(t.getQuestionType()));
 		sql  = sql.replace("#{question_level}",String.valueOf(t.getQuestionLevel()));
 		sql  = sql.replace("#{question_term}",String.valueOf(t.getQuestionTerm()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("t_s_userquestion",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
