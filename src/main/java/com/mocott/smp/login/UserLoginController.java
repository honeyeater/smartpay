...
public class UserLoginController extends BaseController{
	...
	@ResponseBody
	public AjaxJson checkuser(FrontUserRegisterEntity user, HttpServletRequest req) {
		...
		if (StringUtils.isEmpty(randCode)) {
			...
			//用户登录验证逻辑
			FrontUserRegisterEntity u = frontUserRegisterServiceI.checkUserExits(user);
			if (u == null) {
				j.setMsg(mutiLangService.getLang("common.username.or.password.error"));
				j.setSuccess(false);
				return j;
			}
			if (u != null && !"-1".equalsIgnoreCase(u.getValidFlag())) {
				// 处理用户有多个组织机构的情况，以弹出框的形式让用户选择
				Map<String, Object> attrMap = new HashMap<String, Object>();
				j.setAttributes(attrMap);

                session.setAttribute(ResourceUtil.CURRENT_CLINET_USER, "2");
...
//	}
     ...
     * @param orgId 组织主键
     */
    private void saveLoginSuccessInfo(HttpServletRequest req, FrontUserRegisterEntity user, String orgId) {
    	String message = null;
...
//        user.setDepartid(orgId);
		...
		session.setAttribute(ResourceUtil.FRONT_CLINET_USER, user);
        ...
        FrontClient clientOld = FrontClientManager.getInstance().getClient(session.getId()+"front");
		if(clientOld == null || clientOld.getUser() ==null ||user.getUserName().equals(clientOld.getUser().getUserName())){
            ...
            FrontClientManager.getInstance().removeClinet(session.getId()+"front");
			session.invalidate();
	...
	public String login(ModelMap modelMap,HttpServletRequest request,HttpServletResponse response) {
		...
		if (user != null) {
            ...
            modelMap.put("currentOrgName", "客户");
...
//			}
			...
			Cookie cookie = new Cookie("INDEXSTYLE", "COMMON");
			//设置cookie有效期为一个月
...
//              return sysTheme.getIndexPath();
	...
	public ModelAndView logout(HttpServletRequest request) {
		...
		FrontClientManager.getInstance().removeClinet(session.getId()+"front");
		session.invalidate();
		ModelAndView modelAndView = new ModelAndView(new RedirectView("userLoginController.do?login"));
		return modelAndView;
	}
